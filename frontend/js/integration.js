...
//     +'<div style="font-size:12px;color:#374151;margin-top:4px">Password: <strong style="font-family:monospace;color:#166534;font-size:14px">'+pass+'</strong></div>'
...
function admDelCon(cid){
  ...
  var filtered=list.filter(function(x){return x.id!==cid;});
  iSv(IK.cn,filtered);

  // 2. Delete from Supabase hcp_clinicians
  if(typeof SUPABASE_URL!=='undefined'&&typeof SUPABASE_KEY!=='undefined'&&SUPABASE_URL&&SUPABASE_KEY&&c.id){
    ...
    .then(function(res){
      ...
      console.log('[HCP Supabase] ✓ Consultant deleted:',c.hcpEmail);
    })
    .catch(function(err){
      console.error('[HCP Supabase] ✗ Delete failed:',err.message);
      intToast('warn','Supabase delete failed','Removed locally only — check console','Admin');
    });
  }

  // 3. Revoke HCP access list entry (hcp-access-control.js)
  if(typeof hcpRevokeAccess==='function'&&c.hcpEmail){
    hcpRevokeAccess(c.hcpEmail);
  }

  // 4. Remove from HCP credentials store
  if(c.hcpEmail){
    var creds=iLd('evh_hcp_creds',{});
    delete creds[c.hcpEmail];
    iSv('evh_hcp_creds',creds);
  }
...
function admRCon(){
  ...
  tb.innerHTML=list.map(function(c){
    ...
    return '<tr>'
      ...
      +'<td style="white-space:nowrap">'
        +'<button onclick="admOpenEdit(\''+c.id+'\')" style="padding:3px 8px;background:#1E40AF;color:#fff;border:none;border-radius:5px;font-size:10px;cursor:pointer;margin-right:3px">&#x270F; Edit</button>'
        +'<button onclick="admTogCon(\''+c.id+'\')" style="padding:3px 8px;background:'+(c.active?'#7F1D1D':'#166534')+';color:#fff;border:none;border-radius:5px;font-size:10px;cursor:pointer;margin-right:3px">'+(c.active?'Deactivate':'Activate')+'</button>'
        +'<button onclick="admDelCon(\''+c.id+'\')" style="padding:3px 8px;background:#450a0a;color:#ffffff;border:1px solid #ffffff;border-radius:5px;font-size:10px;cursor:pointer">&#x1F5D1; Delete</button>'
      +'</td>'
      +'</tr>';
  }).join('');
}
...
// Seed dummy consultants & slots if none exist (for demo)
function seedDummyData(){
  var cons=iLd(IK.cn,[]);
