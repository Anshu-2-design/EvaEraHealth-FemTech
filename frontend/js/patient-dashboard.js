...
//   .then(function(rows) {
//     var visits = rows
//       .filter(function(r) { return r.created_at && r.scores && r.scores.composite != null; })
//       .map(function(r) {
...
//         };
//       });
//     console.log('[VisitGraph] Supabase visits:', visits.length);
//     callback(visits);
//   })
//   .catch(function(err) {
//     console.warn('[VisitGraph] Failed:', err.message);
//     callback([]);
//   });
// }
...
//   /* async: populate visit graph after render */
...
//   /* Fetch visit history from Supabase only */
//   _pdFetchSupabaseVisitHistory(authId, function(visits) {
//     console.log('[VisitGraph] visits from Supabase:', visits.length);
//     if (visits.length >= 2) {
//       slot.innerHTML = _pdVisitGraphHTML(visits);
//     }
//   });
// }
...
//     + '</div></div>'
//     + '<div id="pd-visit-graph-slot"></div>'
//     + '</div>';
// }
...
// /* EXPOSE GLOBALLY
//    FIX 2: Do NOT expose checkReturningPatient from this file.
//    The authoritative version with full Supabase routing lives in consent.js
//    and is already assigned to window.checkReturningPatient there via the
//    legacy shim. Overriding it here with the localStorage-only version broke
//    the login flow — returning patients would skip Supabase routing entirely. */
...
// window._pdLocalVisitHistory         = _pdLocalVisitHistory;  /* kept for console diagnostics */
...
function _pdFetchSupabaseVisitHistory(authId, callback) {
  ...
  .then(function(rows) {
    var visits = rows
      ...
      .map(function(r) {
        /* Combine login_date (YYYY-MM-DD) + login_time (HH:MM:SS) into a datetime */
